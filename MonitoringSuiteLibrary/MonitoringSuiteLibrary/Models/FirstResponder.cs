    ...
    public class FirstResponder
    {
        ...
        public bool IsVitalsAndLocationCurrent
        {
            get
            {
                ...
                if (Vitals.Value.Timestamp.Date != DateTime.Today || Vitals.Value.Timestamp.ToUniversalTime() < DateTime.UtcNow - decayTime)
                {
                    return false;
                }

                if (Location.Value.Timestamp.Date != DateTime.Today || Location.Value.Timestamp.ToUniversalTime() < DateTime.UtcNow - decayTime)
                {
                    return false;
                }
